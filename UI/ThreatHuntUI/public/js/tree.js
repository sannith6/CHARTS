...
function getTreeData(oldurl) {
        d3.select('.legends').selectAll('*').remove();
        d3.select('.cichart').selectAll('*').remove();
        $(".loadertest").html(loaderdiv);


        var a =oldurl.split("=")[1]
        var newid = a.split('&')[0]

        // var rx = new RegExp(key + "=(.*?)\\s+--");

        // var values = rx.exec(oldURL);

        var url = `http://10.10.110.198:8000/get_tree_data?id=${newid}&graph=fe_2&graph_name=firstenergy_2`
        ...
        request.onload = function () {
            if (this.readyState == this.DONE) {
                ...
                treeFunction(JSON.parse(this.response))
            }
        };
        request.send();
}
 ...
 function treeFunction(treeData) {
