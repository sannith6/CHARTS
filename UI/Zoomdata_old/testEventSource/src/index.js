...
let mockData = [
    {
        src: '10.11.36.20',
        data: {
            date: [1, 4, 6, 9, 12, 13, 14, 15, 17, 19, 20, 22, 24, 26],
...
function createChartTable(receivedData, chartContainer) {
    ...
    receivedData.map((data, i) => {
        ...
        tr.onclick = function (){ rowClickHandler(tr.id)}
        ...
        document.getElementById(`tr${i}`).innerHTML = `<td id="tr${i}src" style="color: #4d90fe;  padding-left: 10px;">${data.src}</td>
...
controller.update = data => {
    ...
    createChartTable(mockData,chartContainer)
};

function rowClickHandler(id) {
    console.log(id)
    var source = document.getElementById(`${id}src`).innerHTML
    // var idInput = document.getElementById('idInput').value
    document.getElementById('idInput').value = source
    // console.log(idInput)
}
