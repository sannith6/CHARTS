...
var contextMenu = function (m, k) {
    return `<ul class="menu" ><li id="selectLinked" ><a tabindex="-1" href="${getLink(m, k)}">Threat Hunt</a></li><li id="selectLinked" ><a tabindex="-1" href="${getDashboard(m, k)}"">Search</a></li></ul>`
}
...
function getDevicesData(response) {
    // console.log(response ,"getDevicedata response")
...
function reduceDataSource(response) {
    ...
    var reducedData = []
    console.log(reducedData, "new response")
    response.map((k) => {
        if (reducedData.length > 0) {
            var elm = reducedData.filter((t) => t["Source Type"] === k["Source Type"] && t["Dst IP"] === k["Dst IP"])
...
const randomScore = ['88', '66', '70', '78', '97', '51', '63', '63', '65', '69', '65', '70', '68', '62', '61', '66']
...
controller.update = data => {
    ...
    if (data.length > 0) {
        ...
        reducedDataSource.map((item, index) => {
            ...
            // if (!highestScore.hasOwnProperty(item[3]) && !highestScore.hasOwnProperty(item[9])) {
            //     scoreIp = getRandomScore(index),
            //         scoreUser = getRandomScoreuser(index)
            // } else {
            //     if (highestScore.hasOwnProperty(item[3]) && !highestScore.hasOwnProperty(item[9])) {
            //         scoreUser = getRandomScoreuser(index),
            //             scoreIp = highestScore[item[3]]
            //     }
            //     else if (highestScore.hasOwnProperty(item[9]) && !highestScore.hasOwnProperty(item[3])) {
            //         scoreIp = getRandomScore(index),
            //             scoreUser = highestScore[item[9]]
            //     }
            //     else {
            //         scoreUser = highestScore[item[9]]
            //         scoreIp = highestScore[item[3]]
            ...
            // }
                ...
                response.push({
                    ...
                    "IP SRC ADDR": item[4],
                    "Score_IP": getRandomScore(index),
                    "Source Type": item[2],
                    "Userid": item[3],
                    "Score_User": getRandomScoreuser(index),
                    "Dst IP": item[5],
                    "Shodan ISP": item[8],
                    "Shodan Hostname": item[9],
                    "Shodan Country": item[10],
                    "Shodan Port": null,
                    "Shodan Product": null,
                    // "Timestamp": item[1],
                    // "Alerts Description": "Exceeded Score Threshold",
                    ...
                    // "Score_IP": getRandomScore(index),
                    // "Source Type": item[2],
                    // "Userid": item[3] === null ? '-' : item[3],
                    // "Score_User": getRandomScoreuser(index),
                    // "Dst IP": item[1] === null ? '-' : item[],
                    // "Shodan ISP": item[5] === null ? '-' : item[5],
                    // "Shodan Hostname": item[4] === null ? '-' : item[4],
                    // "Shodan Country": null,
                    // "Shodan Port": item[6] === null ? '-' : item[6],
                    // "Shodan Product": item[7] === null ? '-' : item[7],
                })
