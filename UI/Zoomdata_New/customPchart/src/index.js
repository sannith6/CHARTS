...
$(document).ready(function () {
    ...
    // http://10.10.110.248:8080/zoomdata/visualization/5b2b9d19bd8cd1acdde7a213+5d54eef19c0c775e11bad0ee?inheritSourceId=5d54e6899c0c775e11bacfad&filters=[{%22editable%22:true,%22operation%22:%22IN%22,%22label%22:%22Ip%20Src%20Addr%22,%22path%22:%22ip_src_addr%22,%22value%22:[%22192.168.0.76%22]}]&from=+$now_-365_day&to=+$now&timeField=event_time1?__target=embedded&key=v4vUkLu7a0

    console.log("WINDOW LOCATION >>>>", window.location.href)
    var x = window.location.href
    var k = x.split("%22")
    var ip_src_addr = k && k[17] ? k[17] : ''
    console.log(k[17])
    var recurringEvents = {}
    ...
    function displayRecurringEvents(data) {
        ...
        data.map((item) => {
            ...
            if (item.activity.toUpperCase() === 'DOWNLOAD' || item.activity.toUpperCase() === 'UPLOAD') {
                extraData = `<span style="font-weight:bold;">IP : </span><span>${item.emailSize}</span><br/>
            ...
            <span style="font-weight:bold;">Email Size : </span><span>1500</span><br/><br/>
            `
    ...
    function onClickActivityTimeline(data) {
        console.log(data, "click on img")
        console.log("All Datas >> ", recurringEvents[data[0]])
        var extraData = ''
        if (data[1] === 'DOWNLOAD' || data[1] === 'UPLOAD') {
            extraData = `<span style="font-weight:bold;">IP : </span><span>${data[4]}</span><br/>
            ...
            <span style="font-weight:bold;">Reciever : </span><span>${data[7]}</span><br/>
    ...
    controller.update = data => {
        ...
        if (data.length > 0) {
            ...
            // logoff,logon,SEND,RECEIVE,Upload,Download
            data.map((item) => {
                if (item[0] === 'logoff' || item[0] === 'logon' || item[0] === 'SEND' || item[0] === 'RECEIVE' || item[0] === 'Upload' || item[0] === 'Download') {
                    if (item[0] !== '-' && ip_src_addr === item[4]) {
                        responses.push({
                            "userid": item[8] ? item[8] : '',
                            "date": item[3] ? item[3] : '',
                            "activity": item[0] ? item[0] : '',
                            "destinationDomain": item[1] ? item[1] : '',
                            "emailSize": item[4] ? item[4] : '',
                            "recievedBytes": item[5] ? item[5] : '',
                            "reciever": item[2] ? item[2] : '',
                            "reportingHost": item[6] ? item[6] : '',
                            "sender": item[8] ? item[8] : '',
                            "sentBytes": item[7] ? item[7] : '',
                        })
                        dates.push(new Date(item[3]))
                    }
                }
            })
