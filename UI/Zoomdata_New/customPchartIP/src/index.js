...
$(document).ready(function () {
    ...
    var k = x.split("%22")
    var sourceip = k && k[17] ? k[17] : ''
    console.log(k[17], "sourceip")
    var recurringEvents = {}
    ...
    function buildTimeLine() {
        ...
        function createScatterPlot(minDates, maxDates) {
            ...
            console.log(dateDiff , data.length , "dateDiff")

            //var multiplier = 550
            var multiplier = dateDiff < 5 ? (dateDiff < 3 ? (dateDiff === 1 ? 500 : 2000) : 1000) : 100
            ...
            var margin = { top: 20, right: 15, bottom: 60, left: 70 }
                ...
                console.log(height , " heif multiplier")
            var x = d3.scalePoint()
                .domain(statuses)
                .range([0, width]);
    ...
    function displayRecurringEvents(data) {
        ...
        data.slice(0, 100).map((item) => {
            ...
            if (item.activity.toUpperCase() === 'DOWNLOAD' || item.activity.toUpperCase() === 'UPLOAD') {
                extraData = `<span style="font-weight:bold;">IP : </span><span>${item.destinationDomain}</span><br/>
    ...
    function onClickActivityTimeline(data) {
        ...
        if (data[1] === 'DOWNLOAD' || data[1] === 'UPLOAD') {
            extraData = `<span style="font-weight:bold;">IP : </span><span>${data[3]}</span><br/>
    ...
    controller.update = data => {
        ...
        if (data.length > 0) {
            ...
            var dates = []
            data.map((item) => {
                console.log("ACTIVITY TIMELINE DATA >>>>> ", item[2])
                if (item[2] === 'logoff' || item[2] === 'logon' || item[2] === 'SEND' || item[2] === 'RECEIVE' || item[2] === 'Upload' || item[2] === 'Download') {
                    if (sourceip === item[6]) {
                        responses.push({
                            "userid": item[7] ? item[7] : '',
                            "date": item[1] ? item[1] : '',
                            "activity": item[2] ? item[2] : '',
                            "destinationDomain": item[6] ? item[6] : '',
                            "emailSize": item[0] ? item[0] : '',
                            "recievedBytes": item[3] ? item[3] : '',
                            "reciever": item[3] ? item[3] : '',
                            "reportingHost": item[5] ? item[5] : '',
                            "sender": item[4] ? item[4] : '',
                            "sentBytes": item[4] ? item[4] : '',
                        })
                        dates.push(new Date(item[1]))
                    }
                }
            })
